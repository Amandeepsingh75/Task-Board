...
type Props = {
  todoArr: Task[];
  deleteTask: (index: number, column: "todo") => void;
  onDragstart: (e: React.DragEvent, index: number, column: "todo") => void;
};

const Todo: React.FC<Props> = ({ todoArr, deleteTask, onDragstart }) => {
  return (
    <div className="border border-gray-300 p-6 h-[80vh] overflow-y-auto">
      <div className="text-xl font-semibold border-b pb-2">Todo List</div>

      {todoArr.length === 0 && (
        <p className="text-gray-400 mt-4">No tasks in Todo</p>
      )}
